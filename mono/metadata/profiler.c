...
output_profile (GList *funcs)
{
	...
	for (tmp = funcs; tmp; tmp = tmp->next) {
		...
		fprintf (poutput, "%7I64u ", (guint64)p->count);
	...
	fprintf (poutput, "Total number of calls: %I64u \n", (gint64)total_calls);
}
...
output_newobj_profile (GList *proflist)
{
	...
	for (tmp = proflist; tmp; tmp = tmp->next) {
		...
		for (tmps = sorted; tmps; tmps = tmps->next) {
			...
			fprintf (poutput, "    %8I64u KB %8I64u %-48s\n", (ainfo->mem / 1024), ainfo->count, buf);
		}
		/* callers */
		output_callers (mp);
	}
	fprintf (poutput, "Total memory allocated: %I64u KB\n", total / 1024);
}
...
simple_allocation (MonoProfiler *prof, MonoObject *obj, MonoClass *klass)
{
	...
	fprintf(poutput, "simple_allocation klass->name=\"%s\" alloc_info->count=%I64u  alloc_info->mem = %I64u\n", 
		klass->name , tmp->count , tmp->mem );
}

static 
void   simple_profile_gc  (MonoProfiler *prof, MonoGCEvent event, int generation)
{
	fprintf(poutput , "simple_profile_gc event = %d  gen = %d\n" , event , generation );
} 
static 
void simple_profile_gc_resize   (MonoProfiler *prof, gint64 new_size)
{
	fprintf(poutput , "simple_profile_gc_resize newsize = %I64d\n" , new_size );
}

static 
void simple_profile_gc_move(MonoProfiler *prof, void **objects, int num)
{
	fprintf(poutput, "simple_profile_gc_move num = %d\n", num);
}
static void
...
mono_profiler_install_simple (const char *desc)
{
	...
	if (desc) {
		...
		for (ptr = args; ptr && *ptr; ptr++) {
			...
			if (!strcmp (arg, "time"))
				...
				flags |= MONO_PROFILE_GC | MONO_PROFILE_GC_MOVES;
	...
	if (flags & MONO_PROFILE_ALLOCATIONS)
		flags |= MONO_PROFILE_ENTER_LEAVE | MONO_PROFILE_EXCEPTIONS | MONO_PROFILE_GC | MONO_PROFILE_GC_MOVES;
	if (!flags)
		flags = MONO_PROFILE_ENTER_LEAVE | MONO_PROFILE_ALLOCATIONS | MONO_PROFILE_JIT_COMPILATION | MONO_PROFILE_EXCEPTIONS | MONO_PROFILE_GC | MONO_PROFILE_GC_MOVES;
	...
	mono_profiler_install_gc( simple_profile_gc , simple_profile_gc_resize );
	mono_profiler_install_gc_moves(simple_profile_gc_move);
